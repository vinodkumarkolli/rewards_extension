...
def get_monthly_data(conditions, values, filters):
	...
	if aggregation_fact == "Sale Amount":
		...
		select_fields = "sr.distributor as distributor, sr.agent_code as agent_code"
	elif aggregation_fact == "Customer Count":
		...
		select_fields = "sr.distributor as distributor, sr.agent_code as agent_code, srli.company_item as company_item"
	else:
	...
	sql_query = f"""
		...
		GROUP BY
			sr.distributor, sr.agent_code{', srli.company_item' if aggregation_fact != 'Sale Amount' and aggregation_fact and aggregation_fact != 'Customer Count' and aggregation_fact != 'Avg Order Value' else ''}
		ORDER BY
			sr.distributor, sr.agent_code{', srli.company_item' if aggregation_fact != 'Sale Amount' and aggregation_fact and aggregation_fact != 'Customer Count' and aggregation_fact != 'Avg Order Value' else ''}
	"""
